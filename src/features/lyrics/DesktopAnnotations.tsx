...
function resolveOverlaps(
  ...
  for (const grp of groups) {
    const totalH =
      grp.reduce((s, item) => s + item.height, 0) + gap * (grp.length - 1);
    const centerIdeal =
      grp.reduce((s, item) => s + item.idealY, 0) / grp.length;
...
export function DesktopAnnotations({
  annotations,
  activeId
}: {
    ...
    createPortal(
      <>
        {anns.map((ann) => (
          ...
          >
            <AnnotationCardContent annotation={ann} />
          </div>
        ))}
      </>,
      colEl
